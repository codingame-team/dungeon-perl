...
my $dungeon = &create_dungeon($opts);
   ...
   # print Dumper($dungeon);
   # Convertir l'array Perl en JSON
    my $json = encode_json($dungeon);
...
sub dump_dungeon {
    ...
    print "Dungeon value: $dungeon\n";

    # Print the rooms
    print "Rooms:\n";
    for my $room (@{$dungeon->{rooms}}) {
        print "  - Room ID: ", $room->{id}, "\n";
        print "    Description: ", $room->{description}, "\n";
        print "    Connections: ", join(", ", @{$room->{connections}}), "\n";
    }
    print Dumper($dungeon->{rooms});

#    # Print the doors
#    print "Doors:\n";
#    print Dumper($dungeon->{door});
#
#    # Print the cells
#    print "Cell:\n";
#    print Dumper($dungeon->{cell});

    # Print the connections
    print "\nConnections:\n";
    for my $connection (@{$dungeon->{connections}}) {
        print "  - Room 1: ", $connection->{room1}, "\n";
        print "    Room 2: ", $connection->{room2}, "\n";
        print "    Type: ", $connection->{type}, "\n";
    }
}

sub export_dungeon_to_json {
    my ($dungeon, $output_file) = @_;

    # Convert the $dungeon data structure to a Perl data structure
    my $json_data = {
        rooms => $dungeon->{rooms},
        cell => $dungeon->{cell},
        connections => $dungeon->{connections},
        # Add any other relevant data from the $dungeon structure
    };

    # Encode the Perl data structure to JSON
    ...
    print "Dungeon exported to $output_file\n";
}
